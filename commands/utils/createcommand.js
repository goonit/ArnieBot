...
module.exports = class CreateCommand extends Command {
        ...
        super(client, {
            ...
            args: [
                ...
                },
                {
                    ...
                    prompt: 'When triggered, what should the bot respond with? (text, imageurl, or youtube url)',
                    type: 'string',
                    infinite: false
                },
                {
    ...
    async run(msg, args) {
        ...
        if (args.commandtype !== 'text' && args.commandtype !== 'sound' && args.commandtype !== 'image') {
        ...
        if (args.commandtype === 'text') {
            ...
            console.log(`custom text command: ${util.inspect(customCmd)}`);

            customCmd.save().then((result) => {
                console.log(`${channelC(` # ${msg.channel.name}`)}: ${botC(`@CuckBot`)} - ${warningC(result.commandText)} was created by ${userC(msg.author.username)}`);

                msg.reply(`New command '${args.commandtrigger}' was successfully created! '${args.commandtrigger} is now ready to be used!`);
            });
            ...
            console.log(`registerCmd text: ${util.inspect(registerCmd)}`);

        } else if(args.commandtype === 'sound') {
            customCmd.commandType = 'sound';

            if(Date.parse(`01/01/2016 ${args.duration.toString()}`) > Date.parse('01/01/2016 00:00:07')) {
                msg.reply(`The maximum duration for a sound command is 7 seconds!`);
                return;
            }

            console.log(`custom sound command: ${util.inspect(customCmd)}`);
            ...
            registerCmd = new SoundCommand(this.client, customCmd);

            console.log(`registerCmd sound: ${util.inspect(registerCmd)}`);

        } else if (args.commandtype === 'image') {
            customCmd.commandType = 'image';
            customCmd.imageUrl = args.commandresponse;

            console.log(`custom image command: ${util.inspect(customCmd)}`);
            ...
            console.log(`registerCmd image: ${util.inspect(registerCmd)}`);
        }
            ...
            childProcess.on('exit', (code) => {
                ...
                fs.stat(`${resourcesPath}/${cmdNoTrigger}${msg.guild.id}.mp3`, (err, stats) => {
                    ...
                    customCmd.save().then((result) => {
                        ...
                        msg.reply(`New command '${cmdName}' was successfully created! '${args.commandtrigger} is now ready to be used!`);
                    });
                });
            });
        });
